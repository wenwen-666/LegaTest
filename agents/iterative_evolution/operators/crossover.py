...
Generate the merged test class `{crossover_class}` that combines the best tests from both versions and adds tests for uncovered methods."""
    ...
    def _infer_target_class_from_test_name(self, test_name: str) -> str:
        """从测试类名通用推断目标类名"""
        import re
        # 移除测试类后缀：TargetClassTestV1 -> TargetClass, TargetClassTest -> TargetClass
    ...
    def _constructor_signature_matches(self, constructor_node, target_sig: str) -> bool:
        ...
        try:
            ...
            expected_count = len(self._parse_jvm_parameter_types(target_params))
            if expected_count == 0 and target_params.strip():
    ...
    def _method_signature_matches(self, method_node, target_sig: str) -> bool:
        ...
        try:
            ...
            target_param_count = len(self._parse_jvm_parameter_types(target_params))
            if target_param_count == 0 and target_params.strip():
                target_param_count = len([p.strip() for p in target_params.split(',') if p.strip()])
    ...
    def _parse_jvm_parameter_types(self, descriptor: str) -> List[str]:
        """解析JVM方法描述符中的参数类型。"""
        if not descriptor:
            return []
        ...
        while index < len(descriptor):
            ...
            if marker == 'L':
                ...
                if end == -1:
                    break
                class_name = descriptor[index + 1:end].split('/')[-1]
                parameters.append(f"{class_name}{array_prefix}")
                index = end + 1
            elif marker in primitive_types:
                parameters.append(f"{primitive_types[marker]}{array_prefix}")
                index += 1
            else:
                index += 1
    ...
    def _parse_method_signature(self, method_sig: str) -> Dict[str, any]:
        ...
        try:
            ...
            if '(' in method_sig and ')' in method_sig:
                param_part = method_sig.split('(')[1].split(')')[0]
                if param_part.strip():
                    parsed_params = self._parse_jvm_parameter_types(param_part)
                    if parsed_params:
                        method_info['parameters'] = parsed_params
                    else:
